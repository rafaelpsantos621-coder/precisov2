...
export default function GalleryView({ query }: GalleryViewProps) {
  ...
  const handleStartAnalysis = async () => {
    ...
    if (pendings.length === 0) { alert('Nao ha registros pendentes.'); return; }
    setIsAnalyzing(true);
  ...
  const exportToPDF = () => {
    if (filteredSpecimens.length === 0) { alert('Nao ha dados para exportar.'); return; }
    const doc = new jsPDF();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex bg-slate-200/50 p-1 rounded-2xl w-full sm:w-fit overflow-x-auto">
        ...
        <div className="flex gap-2">
          <button onClick={handleStartAnalysis}
            ...
            className={cn("p-2.5 rounded-xl font-bold transition-all flex items-center gap-2 px-4 shadow-lg",
          ...
          <button onClick={exportToPDF} disabled={filteredSpecimens.length === 0}
            className="p-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-50 flex items-center gap-2 px-4 shadow-lg shadow-blue-600/20">
            <Download size={18} /><span className="text-sm font-bold">PDF</span>
          </button>
          <button onClick={() => setIsConfirmingExclusion(true)} disabled={specimens.length === 0}
            className="p-2.5 bg-white border border-red-200 text-red-600 rounded-xl hover:bg-red-50 disabled:opacity-50 flex items-center gap-2 px-4">
            <Trash2 size={18} /><span className="text-sm font-bold">LIMPAR</span>
          </button>
        </div>
      </div>
      ...
      {filteredSpecimens.length === 0 ? (
        <div className="h-96 flex flex-col items-center justify-center text-slate-400 bg-white rounded-[2.5rem] border border-slate-100">
          ...
          <p className="text-sm">Importe arquivos PDF para comecar.</p>
        </div>
      ) : (
      ...
      {isConfirmingExclusion && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-slate-500 text-center mb-8">Esta acao e irreversivel.</p>
